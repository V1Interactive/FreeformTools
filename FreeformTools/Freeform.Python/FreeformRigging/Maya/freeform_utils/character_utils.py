...
from freeform_utils import fbx_presets
...
def align_root_and_grips(node_list, settings_file_path):
    """
    From an imported animation apply character settings and use rigging to snap grips to hands and animate root linearly between
    0,0,0 and under the pelvis matching Y rotate at the end frame.

    node_list must come from import_file_safe with tag_imported = True, and keep_scene_time = False.
    maya_utils.scene_utils.import_file_safe(file_path, tag_imported=True, keep_scene_time=False, returnNewNodes=True)
    """
    root_joint = rigging.skeleton.get_root_joint(node_list[0])

    character_network = characterize_skeleton(root_joint, "FixRoot")
    rigging.file_ops.load_settings_from_json(character_network.group, settings_file_path)

    skeleton_dict = rigging.skeleton.get_skeleton_dict(root_joint)

    root_component = rigging.rig_components.fk.FK()
    root_component.rig(skeleton_dict, 'root', 'root')
    root_control = root_component.network['controls'].get_connections()[0]
    pm.cutKey(root_control)

    pelvis_component = rigging.rig_components.fk.FK()
    pelvis_component.rig(skeleton_dict, 'center', 'pelvis')
    pelvis_control = pelvis_component.network['controls'].get_connections()[0]
    pelvis_overdriver = pelvis_component.switch_space( pelvis_control, rigging.rig_overdrivers.overdriver.Overdriver, None )


    scene_times = maya_utils.scene_utils.get_scene_times()
    pm.setKeyframe(root_control, time=scene_times[0], ott="linear", itt="linear")
    pm.currentTime(scene_times[-1])
    pm.xform(root_control, ws=True, matrix=pm.xform(pelvis_control, q=True, ws=True, matrix=True))
    root_control.ty.set(0)
    root_control.rx.set(0)
    root_control.rz.set(0)
    pm.setKeyframe(root_control, time=scene_times[-1], ott="linear", itt="linear")

    grip_r_component = rigging.rig_components.fk.FK()
    grip_r_component.rig(skeleton_dict, 'right', 'grip')
    right_hand_joint = skeleton_dict['right']['arm']['end']
    grip_r_control = grip_r_component.network['controls'].get_connections()[0]

    rigging.skeleton.zero_skeleton_joints([x for x in node_list if type(x) == pm.nt.Joint])
    grip_r_component.zero_rigging()
    grip_r_overdriver = rigging.rig_overdrivers.overdriver.Overdriver()
    grip_r_overdriver.rig(grip_r_component.network['component'].node, grip_r_control, [right_hand_joint], False)


    grip_l_component = rigging.rig_components.fk.FK()
    grip_l_component.rig(skeleton_dict, 'left', 'grip')
    left_hand_joint = skeleton_dict['left']['arm']['end']
    grip_l_control = grip_l_component.network['controls'].get_connections()[0]

    rigging.skeleton.zero_skeleton_joints([x for x in node_list if type(x) == pm.nt.Joint])
    grip_l_component.zero_rigging()
    grip_l_overdriver = rigging.rig_overdrivers.overdriver.Overdriver()
    grip_l_overdriver.rig(grip_l_component.network['component'].node, grip_l_control, [left_hand_joint], False)

    grip_r_overdriver.bake_and_remove()
    grip_r_component.bake_and_remove()
    grip_l_overdriver.bake_and_remove()
    grip_l_component.bake_and_remove()
    root_component.bake_and_remove()
    pelvis_overdriver.bake_and_remove()
    pelvis_component.bake_and_remove()
    
    maya_utils.scene_utils.re_export_from_import(root_joint, fbx_presets.FBXAnimation)